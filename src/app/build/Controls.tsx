...
type Props = Pick<
  ...
  "wheels" | "decks" | "texture"
> & {
  className?: string;
};

export default function Controls({ wheels, decks, texture, className }: Props) {
    ...
    <div className={clsx("flex flex-col gap-6", className)}>
      ...
      <Options title="Trucks" selectedName={selectedTruck?.uid}>
        {texture.map((texture) => (
          <Option
            key={texture.uid}
            colorField={texture.colors}
            selected={texture.uid === selectedTruck?.uid}
            onClick={() => setTruck(texture)}
          >
            {texture.uid?.replace(/-/g, " ")}
          </Option>
        ))}
      </Options>
      <Options title="Bolts" selectedName={selectedBolt?.uid}>
        {texture.map((texture) => (
          <Option
            key={texture.uid}
            colorField={texture.colors}
            selected={texture.uid === selectedBolt?.uid}
            onClick={() => setBolt(texture)}
          >
            {texture.uid?.replace(/-/g, " ")}
          </Option>
        ))}
