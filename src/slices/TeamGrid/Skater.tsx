...
export default function Skater({ skater, index }: Props) {
  ...
  const scrubbleColor = colors[index]
  return (
    <div className="skater group relative flex flex-col items-center gap-4">
        ...
        />
        <SkaterScribble className={clsx("relative", scrubbleColor)} />
        <PrismicNextImage 
        ...
        <ButtonLink field={skater.data.customizer_link} size="sm">Build their beast</ButtonLink>
    </div>
  )
}